...
export interface DatePickerProps
  ...
  dateParser?: (value: string) => LocalDate;
...
export const DatePicker = forwardRef<HTMLInputElement, DatePickerProps>(
  (props: DatePickerProps, ref) => {
    ...
    const dateFormatter = useMemo(
      () =>
        DateTimeFormatter.ofPattern(dateFormat).withLocale(
          new Locale(finalLocale)
        ),
      [dateFormat, finalLocale]
    );
    ...
    const [_value, setValue] = useUncontrolled<LocalDate | null>({
      value,
      defaultValue,
      finalValue: null,
      onChange,
    ...
    const [calendarMonth, setCalendarMonth] = useState(
      (_value && YearMonth.from(_value)) ||
        initialMonth ||
        YearMonth.now(ZoneId.UTC)
    );

    const [focused, setFocused] = useState(false);
    const [inputState, setInputState] = useState(
      _value instanceof LocalDate
        ? upperFirst(_value.format(dateFormatter))
        : ""
    );
    ...
    useEffect(() => {
      ...
      if (value instanceof LocalDate && !focused) {
        setInputState(upperFirst(value.format(dateFormatter)));
      }
    }, [value, focused, dateFormatter]);

    const handleValueChange = (date: LocalDate) => {
      setValue(date);
      setInputState(upperFirst(date.format(dateFormatter)));
    ...
    const parseDate = (date: string) =>
      dateParser ? dateParser(date) : _parseDate(date);
    ...
    const setDateFromInput = () => {
      ...
      setInputState(upperFirst(date.format(dateFormatter)));
      setCalendarMonth(YearMonth.from(date));
    };
        ...
        <Calendar
          ...
          value={
            _value instanceof LocalDate
              ...
              ? LocalDate.parse(_value, dateFormatter)
              : null
          }
