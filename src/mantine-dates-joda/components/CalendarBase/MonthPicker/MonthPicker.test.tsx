...
describe("@mantine/core/MonthPicker", () => {
  ...
  it("renders correct amount of controls", () => {
    ...
    const { container: disabled } = render(
      <MonthPicker
        {...defaultProps}
        minDate={LocalDate.of(2021, 2, 1)}
        maxDate={LocalDate.of(2021, 6, 1)}
      />
    );
  ...
  it("calls onYearChange when next/previous buttons are clicked", async () => {
    ...
    const { container } = render(
      <MonthPicker {...defaultProps} year={2031} onYearChange={spy} />
    );
  ...
  it("adds active styles to selected month", () => {
    ...
    expect(container.querySelector(ACTIVE_CONTROL_SELECTOR)?.textContent).toBe(
      "Dec"
    );
  });
  ...
  it("calls onChange when month control button is clicked", async () => {
    const spy = jest.fn();
    const { container } = render(
      <MonthPicker {...defaultProps} onChange={spy} />
    );
  ...
  it("sets hasNext/hasPrevious CalendarHeader props based on minDate/maxDate props", () => {
    const { container: withMaxYear } = render(
      <MonthPicker {...defaultProps} maxDate={LocalDate.of(2021, 12, 1)} />
    );
    const { container: withMinYear } = render(
      <MonthPicker {...defaultProps} minDate={LocalDate.of(2021, 12, 1)} />
    );

    expect(
      withMaxYear.querySelectorAll(HEADER_CONTROL_SELECTOR)[0]
    ).not.toBeDisabled();
    expect(
      withMaxYear.querySelectorAll(HEADER_CONTROL_SELECTOR)[1]
    ).toBeDisabled();
    expect(
      withMinYear.querySelectorAll(HEADER_CONTROL_SELECTOR)[0]
    ).toBeDisabled();
    expect(
      withMinYear.querySelectorAll(HEADER_CONTROL_SELECTOR)[1]
    ).not.toBeDisabled();
  });
  ...
  it("passes nextLabel/previousLabel props to CalendarHeader component", () => {
    render(
      <MonthPicker
        {...defaultProps}
        nextYearLabel="next-test"
        previousYearLabel="previous-test"
      />
    );
  ...
  it("calls onNextLevel when level label is clicked", async () => {
    const spy = jest.fn();
    const { container } = render(
      <MonthPicker {...defaultProps} onNextLevel={spy} />
    );
