...
describe("@mantine/dates/CalendarHeader", () => {
  ...
  checkAccessibility([<CalendarHeader {...defaultProps} key={1} />]);
  ...
  it("sets previous control disabled state based on hasPrevious prop", () => {
    const { rerender } = render(
      <CalendarHeader {...defaultProps} hasPrevious={false} />
    );
  ...
  it("sets next control disabled state based on hasNext prop", () => {
    const { rerender } = render(
      <CalendarHeader {...defaultProps} hasNext={false} />
    );
  ...
  it("sets level control tabIndex based on preventLevelFocus prop", () => {
    const { rerender } = render(
      <CalendarHeader {...defaultProps} preventLevelFocus />
    );
  ...
  it("disables level control based on nextLevelDisabled prop", () => {
    const { rerender } = render(
      <CalendarHeader {...defaultProps} nextLevelDisabled />
    );
  ...
  it("has correct displayName", () => {
    expect(CalendarHeader.displayName).toStrictEqual(
      "@mantine/dates/CalendarHeader"
    );
  });
});
