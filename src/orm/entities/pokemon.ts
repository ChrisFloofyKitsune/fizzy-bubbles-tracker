...
import {PokemonContestStat, PokemonGenderOptions, PokemonMoveSourceCategory} from "~/orm/enums";
...
export class Pokemon {
    ...
    get levelBBCode() {
        ...
        const activeLog = this.levelLogs.reduce((prev, current) => current.value > prev.value ? current : prev);
    ...
    get bondBBCode() {
        ...
        const activeLog = this.bondLogs.reduce((prev, current) => current.timestamp >= prev.timestamp ? current : prev);
        return wrapUrlIfLink(activeLog.value, activeLog.sourceUrl);
    }
    ...
    get evolutionLineBBCode() {
        ...
        let result = `${this.evolutionStageOne} ${arrow(this.evolutionStageTwoMethod, this.evolutionStageTwoMethodLink)} ${this.evolutionStageTwo}`;
        if (!this.evolutionStageThree) {
            return result;
        }

        return result + ` ${arrow(this.evolutionStageThreeMethod, this.evolutionStageThreeMethodLink)} ${this.evolutionStageThree}`;
    }
    ...
    // noinspection JSUnusedGlobalSymbols
    get beautifulBBCode() {
        const {total, link} = this.compileContestStat(PokemonContestStat.BEAUTIFUL);
        return wrapUrlIfLink(total, link);
    }
    ...
    get levelUpMovesBBCode() {
        ...
        const currentLevel = this.levelLogs.reduce((current, log) => Math.max(current, log.value), 1);
    ...
    compileContestStat(stat: PokemonContestStat) {
        return this.contestStatsLogs?.reduce(
            ({total, link, id}, statLog) => {
                return {
                    total: total + (statLog.stat === PokemonContestStat.ALL || statLog.stat === stat ? statLog.statChange : 0),
                    link: !id ? statLog.sourceUrl : (statLog.id > id ? statLog.sourceUrl : link),
    ...
    getMovesOfCategory(category: PokemonMoveSourceCategory) {
        return !this.moveLogs ? [] : this.moveLogs.filter(m => m.category === category);
    }

    getMovesBBCodeOfCategory(category: PokemonMoveSourceCategory) {
        const list = this.getMovesOfCategory(category);
        return (!list || list.length === 0) ? '' : list.map(m => wrapUrlIfLink(m.move, m.sourceUrl));
    }
}
