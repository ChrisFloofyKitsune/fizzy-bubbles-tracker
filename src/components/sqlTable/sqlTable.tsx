...
export function SqlTable({ entityMetadata, dataSource, placeholder = defaultPlaceholder, changesPendingCallback = () => { } }: SqlTableProps) {
    ...
    const onRowsChange = useCallback((rows: SqlRow[], { indexes }: RowsChangeData<SqlRow>) => {
        for (const index of indexes) {
            ...
            if (row.state === RowState.EXISTS || row.state === RowState.CHANGED) {
                ...
                delete temp.hash;
                delete temp.state;
                row.state = (sha1(temp) === row.hash) ? RowState.EXISTS : RowState.CHANGED;
            }
        }
        ...
        <>
            <Box sx={(!saveError) ? {} : {
                [`.state-${RowState.NEW}, .state-${RowState.CHANGED}`]: {
                    [`& .${saveError.message.match(/\.(?<prop>\w+)$/)?.groups?.prop}-cell`]: {
                ...
                <DataGrid
                    ...
                    summaryRows={[{}]}
                    rowClass={(row: SqlRow) => ((row && row.state) ? tableStyles.classes[row.state] : '') + ` state-${row.state}`}
            ...
            <Group mt='xs'>
                <Button leftIcon={<AddIcon/>} color='green' onClick={async () => await addRowFunc()}>
                    Add Row
                </Button>
                <Button disabled={(!selectedRows || selectedRows.size === 0)} leftIcon={<DeleteIcon/>} color='red'
                    sx={{marginRight: 'auto'}} onClick={deleteRowFunc}
                >
                    Remove Row(s)
                </Button>
                <Button disabled={!changesPending} leftIcon={<CancelIcon/>} color='yellow' onClick={() => {
                    setRefresher(!refresher);
                }}>
                    Cancel Changes
                </Button>
                <Button disabled={!changesPending} leftIcon={<SaveIcon/>} color='blue' onClick={async () => await saveTableFunc()}>
                    Save Changes
                </Button>
            </Group>
            {
                (!saveError) ? '' :
                    <Paper withBorder mt='xl' sx={(theme) => ({
                        backgroundColor: theme.fn.darken(theme.colors.red[9], 0.5)
                    })}>
                        <Blockquote
                            color='red'
                            cite={<Text weight='bolder' sx={{ color: 'white' }}>{saveError.name}</Text>}
