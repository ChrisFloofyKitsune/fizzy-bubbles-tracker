...
import { DatePickerBase, DatePickerBaseSharedProps } from "@mantine/dates";
import { toUTCDayjs } from "~/toUTCDate";

export interface UTCDatePickerProps
...
const defaultProps: Partial<UTCDatePickerProps> = {
  shadow: "sm",
...
export const UTCDatePicker = forwardRef<HTMLInputElement, UTCDatePickerProps>(
  (props: UTCDatePickerProps, ref) => {
    const {
      value,
      onChange,
      defaultValue,
    ...
    const dateFormat =
      type === "date" ? "YYYY-MM-DD" : inputFormat || theme.dateFormat;
    ...
    const [calendarMonth, setCalendarMonth] = useState<Date | null>(
    ...
    const [inputState, setInputState] = useState(
      _value instanceof Date
        ? upperFirst(toUTCDayjs(_value).locale(finalLocale).format(dateFormat))
        : ""
    );
    ...
    useEffect(() => {
      ...
      if (value instanceof Date && !focused) {
        setInputState(
          upperFirst(toUTCDayjs(value).locale(finalLocale).format(dateFormat))
        );
      }
    }, [value, focused]);
    ...
    const parseDate = (date: string) =>
      dateParser ? dateParser(date) : dayjs.utc(date, dateFormat).toDate();

    const setDateFromInput = () => {
      let date = typeof _value === "string" ? parseDate(_value) : _value;

      if (maxDate && toUTCDayjs(date).isAfter(maxDate)) {
        date = maxDate;
      }

      if (minDate && toUTCDayjs(date).isBefore(minDate)) {
        date = minDate;
      }

      if (toUTCDayjs(date).isValid()) {
        setValue(date);
        setLastValidValue(date);
        setInputState(
          upperFirst(toUTCDayjs(date).locale(finalLocale).format(dateFormat))
        );
    ...
    return (
      ...
      >
        <Calendar
          ...
          month={allowFreeInput ? calendarMonth ?? undefined : undefined}
          initialMonth={
            initialMonth || (_value instanceof Date ? _value : new Date())
          }
          onMonthChange={setCalendarMonth}
          value={_value instanceof Date ? _value : toUTCDayjs(_value).toDate()}
...
UTCDatePicker.displayName = "@mantine/dates/UTCDatePicker";
