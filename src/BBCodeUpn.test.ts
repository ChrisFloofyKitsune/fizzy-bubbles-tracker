...
function testCase(input: string, output: string): BBCodeTestCase {
    return [input.split('\n').map(line => line.trim()).join(''), output.split('\n').map(line => line.trim().replace('<br>', '')).join('')];
}
...
const TEST_CASES: BBCodeTestCase[] = [
    ...
    testCase(
        ...
        `<div style="padding: 3px; border: 1px solid #d8d8d8; font-size: 1em;"><div style="text-transform: uppercase; border-bottom: 1px solid #CCCCCC; margin-bottom: 3px; font-size: 0.8em; font-weight: bold; display: balock;"><span onClick="if (this.parentNode.parentNode.getElementsByTagName('div')[1].getElementsByTagName('div')[0].style.display != '') {this.parentNode.parentNode.getElementsByTagName('div')[1].getElementsByTagName('div')[0].style.display = '';this.innerHTML = '<b>Spoiler: </b><a href=\\'#\\' onClick=\\'return false;\\'>Hide</a>';} else {this.parentNode.parentNode.getElementsByTagName('div')[1].getElementsByTagName('div')[0].style.display = 'none';this.innerHTML = '<b>Spoiler: </b><a href=\\'#\\' onClick=\\'return false;\\'>Show</a>';}"/><b>Spoiler: </b><a href="#" onClick="return false;">show</a></span></div><div class="quotecontent"><div style="display: none;">Snape kills Dumbledore</div></div></div>`
    ),
    testCase(
        `A[sub]2[/sub]`,
        `A<sub>2</sub>`
