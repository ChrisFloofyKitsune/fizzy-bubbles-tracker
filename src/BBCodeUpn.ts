...
type replaceOption = [tag: string, options: {
    type: 'replace';
    open: ((attr: string) => string) | string;
    close: ((attr: string) => string) | string | null;
}]
type contentOption = [tag: string, options: {
    type: 'content';
    replace: (attr: string, content: string) => string;
}]
type ignoreOption = [tag: string, optoins: {
    type: 'ignore';
}]
type optionalAttributeOption = [tag: string, options: {
    type: 'optionalAttribute';
    open: ((attr: string, hadAttr: boolean) => string) | string;
    close: ((attr: string) => string) | string | null;
}]

type tagOption = replaceOption | contentOption | ignoreOption | optionalAttributeOption

function attrTag(bbcTag: string, htmlTag: string, attribute?: string, attrValue?: string): replaceOption {
    return [bbcTag, {
        type: 'replace',
        open: attr => `<${htmlTag} ${attribute ?? bbcTag}="${attr?.replace(`"`, '') ?? attrValue ?? bbcTag}">`,
        close: `</${htmlTag}>`
    }]
}
...
const upnTagOptions: tagOption[] = [
    ...
    ['indent', { type: 'replace', open: '<blockquote><div>', close: '</div></blockquote>' }],
    ['email', {
        type: 'optionalAttribute',
    ...
    ['url', {
        type: 'optionalAttribute',
        open: attr => `<a href="${attr.replace('http://forums.upn', 'https://forums.upn').replace(`"`, '')}" target="_blank">`,
        close: '</a>',
    }],
    ['thread', {
        type: 'optionalAttribute',
        open: (attr, hadAttr) => `<a href="https://forums.upnetwork.net/showthread.php?t=${attr}" target="_blank"${!hadAttr ? '' : ` title="UPNetwork - Thread ${attr}"`}>${hadAttr ? '' : `http://forums.upnetwork.net/showthread.php?t=`}`,
        close: `</a>`
    }],
    ['post', {
        type: 'optionalAttribute',
        open: (attr, hadAttr) => `<a href="https://forums.upnetwork.net/showthread.php?p=${attr}#post${attr}" target="_blank"${!hadAttr ? '' : ` title="UPNetwork - Post ${attr}"`}>${hadAttr ? '' : `http://forums.upnetwork.net/showthread.php?p=${attr}#post`}`,
        close: `</a>`
    }],
    ['list', {
        type: 'replace',
    ...
    ['img', {
        ...
        replace: (attr, content) => {
            ...
            return `<img class="inlineimg" src="${content.replace(/^http:\/\/forums.upn/, 'https://forums.upn')}" alt="${attr || ''}" border="0">`;
        },
    },],
    ['code', {
        type: 'replace',
    ...
    ['quote', {
        ...
        open: attr => {
            ...
            return `<div style="margin:20px; margin-top:5px; "><div class="smallfont" style="margin-bottom:2px">Quote:</div><table cellpadding="6" cellspacing="0" border="0" width="100%"><tbody><tr><td class="alt2" style="border:1px inset">`
                + (citation ? `<div>Originally Posted by <strong>${citation}</strong>${postNumber ?
        ...
        close: attr => `${attr ? `</div>` : ``}</td></tr></tbody></table></div>`
    }],
    attrTag('overline', 'span', `style`, `text-decoration: overline;`),
    ['s', { type: 'replace', open: '<del>', close: '</del>' }],
    ['spoiler', {
        type: 'replace',
        open: `<div style="padding: 3px; border: 1px solid #d8d8d8; font-size: 1em;"><div style="text-transform: uppercase; border-bottom: 1px solid #CCCCCC; margin-bottom: 3px; font-size: 0.8em; font-weight: bold; display: balock;">`
            + `<span onClick="` + (
