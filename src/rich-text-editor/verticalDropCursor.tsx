...
export const VerticalDropCursor = Extension.create<DropCursorOptions>({
  name: "verticalDropCursor",
  ...
  addProseMirrorPlugins() {
    return [verticalDropCursorPlugin(this.options)];
  },
  ...
  extendNodeSchema(extension) {
    ...
    return {
      ...
      verticalDropCursor:
        callOrReturn(
          getExtensionField(extension, "verticalDropCursor", context)
        ) ?? null,
    };
  },
});

export function verticalDropCursorPlugin(
  options: DropCursorOptions = {}
): Plugin {
  return new Plugin({
    view(editorView) {
      return new VerticalDropCursorView(editorView, options);
    },
  });
}

class VerticalDropCursorView {
  width: number;
  color: string;
  ...
  updateOverlay() {
    ...
    if (!$pos.parent.inlineContent) {
      let before = $pos.nodeBefore,
        after = $pos.nodeAfter;
      const vertical = !!$pos.parent.type.spec.verticalDropCursor;
      if (before || after) {
        let nodeRect = (
        ...
        let horizontal = before ? nodeRect.right : nodeRect.left;
        if (before && after)
          top =
            (top +
              (
            ...
            2;
        rect = {
          left: !vertical ? nodeRect.left : horizontal - this.width / 2,
          right: !vertical ? nodeRect.right : horizontal + this.width / 2,
          top: !vertical ? top - this.width / 2 : nodeRect.top,
          bottom: !vertical ? top + this.width / 2 : nodeRect.bottom,
        };
      }
    }
    if (!rect) {
  ...
  scheduleRemoval(timeout: number) {
    ...
    this.timeout = setTimeout(() => this.setCursor(null), timeout) as any;
  }
