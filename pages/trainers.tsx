...
Background: Once upon a time {{name}} set out to be the best like no one ever was...`
    ...
    return <>
        <Stack>
            ...
            <Divider size='lg' />
            <Title order={3} sx={{ textAlign: 'center' }}>{`${selected?.name ?? 'No one'}'s Profile`}</Title>
            {!!selected ? <></> :
                <Button
                    color='green'
                    leftIcon={<AddIcon />}
            ...
            {(!editModeOn || !selected || !repo) ? <></> :
                <EntityEditor
                    ...
                    onUpdate={trainer => {
                        setSelected(trainer);
                        const index = entityList.findIndex(t => t.uuid === trainer.uuid);
            ...
            <BBCodeArea
                ...
                bbCode={selected?.bbcodeProfile.replaceAll('{{name}}', selected.name) ?? ''}
            />
        </Stack>
    </>;
};
